local utils = require("neotest-boost_test.utils")
local lib = require("neotest.lib")
local config = require("neotest-boost_test.config")

---@class neotest-boost_test.Report
---@field _node neotest.Tree
---@field _boost_test_json table
local Report = {}
...
---@param boost_test_data table
---@param node neotest.Tree
---@return neotest-boost_test.Report
function Report:new(boost_test_data, node)
  local obj = { _boost_test_json = boost_test_data, _node = node }
...
function Report:_build_status()
  if self._status == nil then
    local test = self._boost_test_json
...
---@class neotest-boost_test.BoostTestError
---@field failure? string
---@field type? string

---@class neotest-boost_test.ParsedBoosttestError
---@field filename? string
---@field linenum? number
---@field header? string
---@field body string

---@param boost_test_error neotest-boost_test.boost_testError
---@return neotest.Error
function Report:_error_info(boost_test_error)
  local parsed_error = self:_parse_boost_test_error(boost_test_error)

  local neotest_error = {
    message = parsed_error.body,
    pretty_message = self:_format_pretty_error_message(parsed_error),
    colors()[boost_test_error],
  }
...
---@param boost_test_error neotest-boost_test.boost_testError
---@return neotest-boost_test.Parsedboost_testError
function Report:_parse_boost_test_error(boost_test_error)
  local message = boost_test_error.failure
  if message == nil then
  ...
  self._parsed_boost_test_error = {
    filename = filename,
    linenum = linenum,
    header = header,
    body = remaining_message,
  }
  return self._parsed_boost_test_error
end
...
function Report:_build_errors_list()
  if self._errors == nil then
    ...
    end, self._boost_test_json.failures or {})
  end
  return self._errors
end
...
function ReportConverter:make_neotest_results()
  local boost_test_json = self:_read_boost_test_json()
  local results = {}
  for _, testsuite in ipairs(boost_test_json.testsuites) do
...
function ReportConverter:_find_node_by_name(namespace, name)
  ...
  -- --boost_test_filter=A.*: the results will include A::B from file1
...
function ReportConverter:_notify_if_incomplete_results(results)
  ...
  if #missing > 0 then
    local message = string.format(
      [[boost_test executable %s did not produce results for the following tests: %s. 
...
---@private
function ReportConverter:_read_boost_test_json()
  ...
  if not success then
    self:_raise_boost_test_failed()
  end

  return data
end

function ReportConverter:_raise_boost_test_failed()
  local executable = self._spec.command[1]
  local message
  if utils.fexists(executable) then
    message = string.format(
      [[boost_test executable failed to produce a result. Command: %s, exit code: %d, output at: %s\n
    ...
    message = string.format([[boost_test executable at path %s not found.]], executable)
  end
