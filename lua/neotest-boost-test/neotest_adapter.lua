...
local executables = require("neotest-boost-test.executables")
...
local function _raise_nodes_missing_executables(node_names)
  ...
  utils.schedule_error(
    string.format(
      "Some nodes do not have a corresponding BoostTest executable set. Please "
        .. "configure them by marking them and then running :ConfigureBoostTest "
...
---@class neotest-boost-test.NeotestAdapter
...
---@return neotest-boost-test.NeotestAdapter
...
function NeotestAdapter:_make_strategy_for_command(command)
  ...
  return {
    name = "Debug with neotest-boost-test",
