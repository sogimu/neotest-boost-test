...
local GlobalRegistry = require("neotest-boost-test.executables.global_registry")
local ui = require("neotest-boost-test.executables.ui")
...
function M.set_summary_autocmd()
  local group = nio.api.nvim_create_augroup("NeotestBoosttestConfigureMarked", { clear = true })
  nio.api.nvim_create_autocmd("FileType", {
    pattern = "neotest-summary",
    group = group,
    callback = function(ctx)
      local buf = ctx.buf
      nio.api.nvim_buf_create_user_command(buf, "ConfigureBoostTest", M.configure_executable, {})
      if config.mappings.configure ~= nil then
        vim.api.nvim_buf_set_keymap(buf, "n", config.mappings.configure, "", {
          desc = "Select a Boost Test executable for marked tests",
