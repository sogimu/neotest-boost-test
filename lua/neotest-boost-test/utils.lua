...
local runs_dir = Path:new(stddata .. "/neotest-boost-test/runs")
local storage_dir = stddata .. "/neotest-boost_test"
...
---@class neotest-boost_test.mtime
...
---@param lhs neotest-boost_test.mtime First mtime to compare
---@param rhs neotest-boost_test.mtime Second mtime to compare
...
---@return neotest-boost_test.mtime? mtime table for the file at `path`
...
function M.new_results_dir(opts)
  ...
  for _, dir in ipairs(scandir.scan_dir(parent_path.filename, { depth = 1, only_dirs = true })) do
    if dir:match("%/neotest%-boost_test%-run%-%d+$") then
  ...
  for i = opts.history_size, #existing_paths do
    ...
    assert(path:match("%/googletest%-of.*%/neotest%-boost_test%-run%-%d+$"))
  ...
  local new_path = Path:new(parent_path, ("neotest-boost_test-run-%d"):format(new_nr))
  new_path:mkdir({ exist_ok = false })
  symlink(new_path.filename, Path:new(parent_path, "neotest-boost-test-latest").filename)
