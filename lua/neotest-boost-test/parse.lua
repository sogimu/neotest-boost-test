...
local TREESITTER_BOOSTTEST_QUERY = vim.treesitter.query.parse(
  "cpp",
  [[
  ((function_definition
    declarator: (
  ...
  (#any-of? @test.kind "BOOST_AUTO_TEST_CASE" ))
  @test.definition
]]
)
...
local function extract_captures(
  ...
  -- construct in BoostTest used only to group tests together. We set its range
...
local function collect_tests(file_path, source, root)
  ...
  local namespaces, tests = extract_captures(TREESITTER_BOOSTTEST_QUERY, source, root)
